...
class Client:
    def __init__(self):
        ...
        self.key = RSA.generate(1024) #generate an RSA pair
        open(f'{self.name}_key.pem','wb').write(self.key.exportKey()) #save RSA key is key.pem
        self.connect()
    ...
    def send(self):
        ...
        encryptor = PKCS1_OAEP.new(self.key.publickey()) #use to create public key
        encrypted_msg = encryptor.encrypt(bytes(msg, "utf8")) #encrypt the message using publickey
